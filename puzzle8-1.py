...
def main(instr):
	instructions, registers, i, mx = {}, {}, 0, 0
	for line in instr:
		ins, cond = line.split(" if ")
		ins_a = ins.split(" ")
		cond_a = cond.split(" ")
		#make a dictionary of registers
		if ins_a[0] not in registers.keys():
			registers[ins_a[0]] = 0
		if cond_a[0] not in registers.keys():
			registers[cond_a[0]] = 0
		if ins_a[1] == "inc":
			ins_a[1] = "+="
		else:
			ins_a[1] = "-="
		ins_a[0] = "registers['{}']".format(ins_a[0])
		cond_a[0] = "registers['{}']".format(cond_a[0])		
		#make a set of instructions
		instructions[i] = ["".join(ins_a), "".join(cond_a)]
		i += 1
	#we have to go over the instructions twice because we had to build the set of registers before we could operate.
	for v in instructions.values():
		#print(v)
		instruction, condition = v[0], v[1]
		#print("testing {}".format(condition))
		if eval(condition):
			#print("executing {}".format(instruction))
			exec(instruction)
		#print(registers)
		n_mx = max(registers.values())
		mx = n_mx if n_mx > mx else mx
