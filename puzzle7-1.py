...
class Prog:	
	def __init__(self, name=None, weight=None, children=None):
		if name == None:
			raise TypeError("Programs must have a name")
		else:
			self.name = name
		if weight == None:
			raise TypeError("Programs must have a weight")
		else:
		...
		if children == None:
			...
			try:
				self.children = children.split(", ") #it's a string
			except:
				self.children = children #it's not a string
		self.parent = None
		self.fweight = self.weight

	def setParent(self, parent=None):
		if parent == None:
			raise TypeError("You can't add a null parent")
		self.parent = parent
	...
	def setFullWeight(self):
		""" Only call this after all programs have been entered; otherwise it'll record an inaccurate value
		"""
		self.fweight += weighChildren(self.name)
...
def weighChildren(parent=None):
	if parent == None:
		raise TypeError("You must include a parent.")
	#print(progs[parent])
	children = progs[parent].getChildren()
	if children == None:
		return 0
	else:
		weights = []
		for child in children:
			weights.append(progs[child].getWeight() + weighChildren(child))
		return sum(weights)


def getOutlier(parent=None, trueweight=None):
	if parent == None:
		raise TypeError("You must include a parent.")

	if progs[parent].getChildren() == None:
		return -1
	weights = {}
	for k in progs[parent].getChildren():
		weights[k] = progs[k].getFullWeight()
	
	mn, mx = min(weights.values()), max(weights.values())
	if mn == mx:
		return -1
	outlier = mn if [weights.values()].count(mn) == 1 else mx
	dif = -1 if outlier == mn else 1
	wrong = None
	for k,v in weights.items():
		if v == outlier:
			wrong = k
	wgt = progs[wrong].getWeight()
	res = getOutlier(wrong)
	return (wgt - (mx-mn)) * dif if res == -1 else res
...
def main():
	...
	for line in in_a:
		if "->" in line:
			ident, children = line.split(" -> ")
		else:
			ident, children = line, None
		name, weight = ident.split(" ")
		weight = weight[1:-1]
		progs[name] = Prog(name, weight, children)
	
	for k, v in progs.items():
		progs[k].setFullWeight()
		c = v.getChildren()
		if c != None:
			for child in c:
				progs[child].setParent(k)
		#print(progs[k])
	...
	for k,v in progs.items():
		#print(progs[k])
		if v.getParent() == None:
			#print("{} IS THE BASE".format(k))
			base = k
	print(getOutlier(base))
	# weighChildren(base)
